...
//using WCC=VMS.OIS.ARIAExternal.WebServices.Commons.Contracts;
//using PatientIdentifier_Local = VMS.OIS.ARIALocal.WebServices.Document.Contracts.PatientIdentifier;
using VMS.OIS.ARIALocal.WebServices.Document.Contracts;
using System.IO;

namespace catchDose
{
    public class CustomInsertDocumentsParameter_2
    {
        ...
        public static bool PostDocumentData(string patientId, InfoUser user, byte[] binaryContent, string templateName, DocumentType documentType, DocSettings_2 docSet)
        {
           ...
           // bool IsApproved = true;
            ...
            var documentPushRequest = new CustomInsertDocumentsParameter_2
            {
            ...
            if (!response_document.Contains("GatewayError"))
            {
                ...
                */
                // Modifierd to use local DocumentResponse by Luc SIMON
                DocumentResponse documentResponse = JsonConvert.DeserializeObject<DocumentResponse>(response_document);
                if (!object.ReferenceEquals(documentResponse, null))
                {
                        return true;
                }
            }
